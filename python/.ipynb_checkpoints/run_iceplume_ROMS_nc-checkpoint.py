...
def make_input(roms_his_file, roms_river_file, roms_grid_file, tracer1d=True, use_average=False):
    ...
    for i in range(nriver):
        if use_average:
            xloc0 = int(xloc[0, i])
            xloc1 = int(xloc[1, i])+1
            yloc0 = int(yloc[0, i])
            yloc1 = int(yloc[1, i])+1
            traw = fh.variables['temp'][:, :, xloc0:xloc1, yloc0:yloc1]
            sraw = fh.variables['salt'][:, :, xloc0:xloc1, yloc0:yloc1]
            zraw = fh.variables['zeta'][:,    xloc0:xloc1, yloc0:yloc1]
            hraw = grd.vgrid.h[xloc0:xloc1, yloc0:yloc1]
            zrraw = get_zr(zraw, hraw, grd.vgrid)
            titp = np.zeros(traw.shape)
            sitp = np.zeros(sraw.shape)
            tN, zN, xN, yN = traw.shape
            for ti in range(tN):
                for xi in range(xN):
                    for yi in range(yN):
                        titp[ti, :, xi, yi] = np.interp(
                            zr[ti, :, i], zrraw[ti, :, xi, yi], traw[ti, :, xi, yi], left=np.nan)
                        sitp[ti, :, xi, yi] = np.interp(
                            zr[ti, :, i], zrraw[ti, :, xi, yi], sraw[ti, :, xi, yi], left=np.nan)
            titp = np.ma.masked_invalid(titp)
            sitp = np.ma.masked_invalid(sitp)
            tAm2[:, :, i] = titp.mean(axis=(-2, -1))
            sAm2[:, :, i] = sitp.mean(axis=(-2, -1))
        else:
            tAm2[:, :, i] = temp[:, :, i]
            sAm2[:, :, i] = salt[:, :, i]
